...
const handleError = (error) => {
	if (error instanceof ValidationError) {
		console.error(chalk.red.bold('VALIDATION ERROR: ') + chalk.white(error.message));
		if (error.suggestion) {
			console.error(chalk.yellow('Suggestion: ') + chalk.white(error.suggestion));
		}
	} else if (error instanceof GitRepositoryError) {
		console.error(chalk.red.bold('GIT REPOSITORY ERROR: ') + chalk.white(error.message));
		if (error.suggestion) {
			console.error(chalk.yellow('Suggestion: ') + chalk.white(error.suggestion));
		}
	} else if (error instanceof GitNotFoundError) {
		console.error(chalk.red.bold('GIT NOT FOUND: ') + chalk.white(error.message));
		if (error.suggestion) {
			console.error(chalk.yellow('Suggestion: ') + chalk.white(error.suggestion));
		}
	} else if (error instanceof GitCommandError) {
		console.error(chalk.red.bold('GIT COMMAND ERROR: ') + chalk.white(error.message));
		if (error.suggestion) {
			console.error(chalk.yellow('Suggestion: ') + chalk.white(error.suggestion));
		}
		if (debugMode && error.originalError) {
			console.error(chalk.dim('\nOriginal error:'));
			console.error(chalk.dim(error.originalError.stack || error.originalError));
		}
	} else {
		console.error(chalk.red.bold('ERROR: ') + chalk.white(error.message || error));

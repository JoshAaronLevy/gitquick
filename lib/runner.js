...
const getUnstagedFiles = async (message, commit) => {
	...
	try {
		...
		if (fileList.length > 0) {
			const trimmedList = fileList.map(file => file.trim());
			console.log("trimmed file list: ", trimmedList);
			...
			spinner.succeed(chalk.bold(`${stageMessage} found`));
...
const gitCommitStep = async (message, stageMessage) => {
	const spinner = ora("Committing your awesome code...").start();
	...
	try {
		...
		return gitPushStep(message, stageMessage);
	} catch (error) {
		return spinner.fail(
...
const gitPushStep = async (message, stageMessage) => {
	...
	try {
		...
		if (p_1.exitCode === 128) {
			...
			return gitPushUpstream(currentBranch, message, stageMessage);
		} else {
			spinner.fail(
				chalk.red.bold("ERROR! ") +
				chalk.white(
...
const gitPushUpstream = async (currentBranch, message, stageMessage) => {
	const spinner = ora(
